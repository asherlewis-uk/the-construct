...
## PsychProfile Interface

The `PsychProfile` interface is the core diagnostic data structure for emotional state tracking. Every subject in The Construct has a live psychological profile represented as three integer axes on a Recharts radar chart, plus a computed boolean flag that governs critical UI state transitions.

All three numeric fields are integers clamped to the range `0–100`. These values are returned by the Ollama model inside the `psych_profile` JSON object, then normalized and stored in React state as a `PsychProfile` instance.
...
The `Subject` interface defines the complete persona configuration for each interrogation target. Subjects are pre-configured in `src/data/subjects.ts` and loaded into React state when the Admin selects an uplink from the subject switcher. Each subject binds to a specific locally-hosted Ollama model and carries its own hidden system prompt, visual theme, and baseline psychological statistics.
...
The `ChatMessage` interface defines the structure of every entry in the terminal message log. Messages are stored in a React state array (`ChatMessage[]`) within `App.tsx` and rendered sequentially by the `TerminalInterface` component. Each message carries a unique identifier, a role discriminator, display content, a precise timestamp, and an optional psychological snapshot.
...
The `OllamaResponse` interface defines the raw JSON contract returned by the Ollama model **after** the `extractJSON()` utility in `src/services/neuralUplink.ts` isolates the JSON object from potential surrounding text. This interface uses `snake_case` field names to match the naming convention enforced in the subject's system prompt, which the LLM follows when generating its JSON output.

This is an intermediate data structure. It is **not** stored in React state directly. The `NeuralUplink` service parses the raw Ollama output into an `OllamaResponse`, then transforms it into a `PsychProfile` (with the computed `isCritical` flag) and a display string before dispatching to the application state layer.
